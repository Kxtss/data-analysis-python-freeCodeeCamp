...
def calculate_demographic_data(print_data=True):
    # Read data from file
    df = pd.read_csv("Demographic Data Analyzer/adult_data.csv", sep=",")

    # How many of each race are represented in this dataset? This should be a Pandas series with race names as the index labels.
    race_count = df["race"].value_counts()

    # What is the average age of men?
    average_age_men = float(round(df.groupby("sex")["age"].mean().loc["Male"], 1))
    ...
    rich_percentage = float(
        round(
            (df[df["hours-per-week"] == min_work_hours]["salary"] == ">50K").mean()
            * 100,
            1,
        )
    )
    ...
    highest_earning_country_percentage = float(round(country_percentage.iloc[0], 1))
